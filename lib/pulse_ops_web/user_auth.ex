defmodule PulseOpsWeb.UserAuth do
  @moduledoc """
  Session handling and the plugs and `on_mount` hooks that build `current_scope`.

  `:require_organization` is the tenant gate: it resolves the `:org` slug in the
  route, checks membership, and narrows the scope to that organization.
  """

  use PulseOpsWeb, :verified_routes
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      PulseOpsWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      PulseOpsWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule PulseOpsWeb.PageLive do
        use PulseOpsWeb, :live_view

        on_mount {PulseOpsWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{PulseOpsWeb.UserAuth, :require_authenticated}] do
  ...
  def on_mount(:require_organization, %{"org" => slug}, session, socket) do
    socket = mount_current_scope(socket, session)
    ...
    if scope && scope.user do
      case Organizations.fetch_for_user(slug, scope.user) do
        ...
        {:error, :not_found} ->
          {:halt, redirect_with_error(socket, "Organization not found.", ~p"/")}
      end
    else
      {:halt,
       redirect_with_error(socket, "You must log in to access this page.", ~p"/users/log-in")}
    end
  end
