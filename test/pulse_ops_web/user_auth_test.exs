defmodule PulseOpsWeb.UserAuthTest do
  ...
  @remember_me_cookie "_pulse_ops_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, PulseOpsWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      PulseOpsWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_organization" do
    setup %{conn: conn, user: user} do
      organization = organization_fixture(user, %{name: "Acme", slug: "acme"})
    ...
    test "narrows the scope to the organization and the caller's role", %{
      session: session,
    ...
    test "carries the membership role rather than assuming owner", %{conn: conn} do
      owner = user_fixture()
      viewer = user_fixture()
      organization = organization_fixture(owner, %{name: "Shared", slug: "shared"})
      membership_fixture(organization, viewer, :viewer)

      session =
        conn
        |> put_session(:user_token, Accounts.generate_user_session_token(viewer))
        |> get_session()
      ...
      assert socket.assigns.current_scope.role == :viewer
    end

    test "halts for a user who is not a member", %{conn: conn} do
      owner = user_fixture()
      outsider = user_fixture()
      organization = organization_fixture(owner, %{name: "Private", slug: "private"})

      session =
        conn
        |> put_session(:user_token, Accounts.generate_user_session_token(outsider))
        |> get_session()
      ...
      refute socket.assigns.current_scope.organization
    end

    test "answers identically for a missing organization and a forbidden one", %{
      conn: conn,
      session: session
    } do
      outsider = user_fixture()
      ...
      # Same flash and same redirect: the slug's existence must not leak.
      assert forbidden.assigns.flash == missing.assigns.flash
      assert forbidden.redirected == missing.redirected
    end

    test "halts when nobody is logged in", %{conn: conn, organization: organization} do
      session = conn |> get_session()
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        PulseOpsWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
